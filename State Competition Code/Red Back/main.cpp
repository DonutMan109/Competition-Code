...
void autonomous( void ) {
    ...
    turn(-1, 1, 50);            //90 degrees left. if turn isn't enough, increase both 1's by a decimal. if it turns too much decrease by decimal. keep the 50
    ...
    turn(-1, 1, 50);        //90 degress left. intake should face wall, if needed, change the 1's.
    driveStraight(-1.35, 60);   //should back up to the tile right before the tilted cap
    turn(-1, 1, 50);    //another 90 degree left. this is probably gonna have to be fine tuned by decimal points, so change the 1's
